...
#
# Run from the src/vmonitor-mcp-server directory. Requires `uv` and `curl`.
...
echo "==> Starting server on ${BASE} (--auth-debug)"
LOG="$(mktemp -t auth-debug-server.XXXXXX.log)"
uv run python -m greennode.vmonitor_mcp_server.server \
